...
let () =
  ...
  let tycheck_opt = StdOpt.store_true () in
  let compile_opt = StdOpt.store_true () in  
  ...
  OptParser.add opt_parser ~long_name:"compile" compile_opt;
  ...
  | Dynamic_scope_error x ->
     eprintf "!!! AT RUNTIME: Scope error !!!\nVariable %s is unbound.\n" x
  | Static_scope_error x ->
     eprintf "Compile-time: Scope error\nVariable %s is unbound.\n" x
