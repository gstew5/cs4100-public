...
type tok = IF | THEN | ELSE | BEGIN
	   | END | PRINT | SEMI | NUM of int | EQ

exception Parse_failure of tok option * string

(** We use a reference (a mutable memory cell) to 
    record the current list of yet-to-be-parsed tokens; 
    [try_parse] populates this list. *)
...
let get_and_eat_num (_ : unit) : int =
  let tok = getToken () in
  match tok with 
  | NUM n -> eat_num (); n
  | _ -> raise (Parse_failure (Some tok, "not a NUM"))
...
let rec s (_ : unit) : unit =
  ...
  | IF -> eat IF; e (); eat THEN; s (); eat ELSE; s ();
	  print_string "S -> if E then S else S\n"
  | BEGIN -> eat BEGIN; s (); l (); print_string "S -> begin S end\n"
  | PRINT -> eat PRINT; e (); print_string "S -> print E\n"
  | _ -> raise (Parse_failure (Some tok, "in nonterminal S"))

and l (_ : unit) : unit =
  let tok = getToken () in 
  match tok with
  | END -> eat END; print_string "L -> end\n"
  | SEMI -> eat SEMI; s (); l (); print_string "L -> ; S L\n"
...
and e (_ : unit) : unit =
  ...
  | NUM n ->
     eat_num ();
     eat EQ;
     let m = get_and_eat_num ()
     in BatPrintf.printf "E -> NUM(%d) = NUM(%d)\n" n m
  | _ -> raise (Parse_failure (Some tok, "in nonterminal E"))

let try_parse (l : tok list) : (tok option * string) =
  tokList := l;
  try s (); (None, "success")
  with Parse_failure (otok, err) -> (otok, err)
;;
